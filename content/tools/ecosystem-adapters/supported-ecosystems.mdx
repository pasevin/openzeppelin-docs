...
Each adapter implements the subset of the [13 capability interfaces](/tools/ecosystem-adapters/architecture#capability-tiers) that its blockchain supports. This page summarizes what each production adapter provides.
...
The Operator profile is explicitly unsupported — `createRuntime('operator', ...)` throws `UnsupportedProfileError` because `accessControl` factories are not yet implemented.
